...
public class AutorController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<AutorDTO>>> GetAsync()
    {
        ...
        if(autores == null) return NotFound("Autores não encontrados. Por favor, tente novamente!");
    ...
    public async Task<ActionResult<AutorDTO>> PostAsync(AutorDTO autorDTO)
    {
        ...
        var autorRetornoDTO = _mapper.Map<AutorDTO>(autorCriado);
        return new CreatedAtRouteResult("ObterIdAutor" , new { id = autorRetornoDTO.IdAutor } , autorRetornoDTO);
    }
    #endregion

    #region PUT
    [HttpPut("AtualizarAutor/{id:int:min(1)}")]
    public async Task<ActionResult<AutorDTO>> PutAsync(int id , AutorDTO autorDTO)
    {
    ...
    public async Task<ActionResult<AutorDTO>> DeleteAsync(int id)
    {
        ...
        var autorExcluidoDTO = _mapper.Map<CategoriasDTO>(autorExcluido);
